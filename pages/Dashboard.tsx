...
const Dashboard: React.FC<DashboardProps> = ({ user, onUpdateUser }) => {
  ...
  return (
    <div className="px-5 py-8 space-y-6 animate-fade-in">
      {/* Header */}
      <header className="flex justify-between items-start">
        ...
        <div className="flex gap-3">
          <button className="glass p-2 rounded-full text-gold hover:bg-gold/10 transition-colors"><Settings size={20} /></button>
          <button className="glass p-2 rounded-full text-gold hover:bg-gold/10 transition-colors"><UserIcon size={20} /></button>
        </div>
      </header>
      ...
      {/* Hero Archetype Card */}
      <div className={`relative p-6 rounded-3xl overflow-hidden glass border-amber-500/30 bg-gradient-to-br ${phaseInfo.gradient} shadow-2xl transition-all duration-700`}>
        ...
        <div className="relative z-10 space-y-4">
          ...
          <div className="space-y-1">
            ...
            <div className="h-1 w-full bg-white/10 rounded-full overflow-hidden">
              <div className="h-full bg-amber-500 transition-all duration-1000" style={{ width: '80%' }}></div>
            </div>
          </div>
          ...
          <button className="text-[10px] uppercase tracking-tighter text-amber-500 border-b border-amber-500/30 pb-0.5 hover:text-amber-400 transition-colors">Edit Cycle Info</button>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar py-1">
          {Object.entries(SOIL_STATE_DETAILS).map(([state, details]) => (
            <button 
              key={state}
              className={`flex-shrink-0 px-4 py-2 rounded-full text-[10px] font-bold border transition-all ${latestSoil === state ? 'bg-amber-500/20 border-amber-500 text-amber-400' : 'glass border-gold/10 text-gold/50'}`}
        ...
        <div className="glass p-4 rounded-xl border-gold/10 bg-gold/5 italic text-xs leading-relaxed text-cream/90 animate-fade-in">
      ...
      <div className="glass p-6 rounded-2xl relative border-amber-500/20 space-y-4 shadow-xl overflow-hidden">
        ...
        {loadingGuidance ? (
          ...
          <p className="text-sm leading-relaxed italic text-cream/90 transition-all duration-500">
      ...
      <div className="glass p-6 rounded-2xl space-y-4 border-gold/10">
            ...
            return (
              <div key={id} className="space-y-2 group">
                <div className="flex justify-between items-center">
                  <h5 className="text-xs font-bold text-gold uppercase tracking-wider group-hover:text-amber-500 transition-colors">{wound?.name}</h5>
        ...
        <button className="text-[10px] uppercase text-amber-500 border-b border-amber-500/20 block pt-2 hover:text-amber-400 transition-colors">Change Root Wounds</button>
      </div>
      ...
      {showCheckInModal && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 animate-fade-in">
          ...
          <div className="relative w-full max-w-md glass border-t border-gold/20 sm:border rounded-t-3xl sm:rounded-3xl p-8 animate-slide-up">
            ...
            <div className="grid grid-cols-1 gap-3 max-h-[60vh] overflow-y-auto no-scrollbar pb-4">
              {Object.entries(SOIL_STATE_DETAILS).map(([state, details]) => (
                <button 
                  key={state}
                  onClick={() => handleSaveCheckIn(state as SoilState, 5)}
                  className="flex items-center gap-4 p-4 rounded-xl glass border border-gold/10 hover:border-amber-500/50 hover:bg-amber-500/5 transition-all text-left group"
                >
                  <span className="text-3xl group-hover:scale-110 transition-transform">{details.icon}</span>
                  <div>
                    <h5 className="font-bold text-sm text-gold uppercase tracking-wide">{details.label}</h5>
                    <p className="text-[10px] opacity-60 leading-tight mt-1">{details.guidance}</p>
                  </div>
                </button>
              ))}
            </div>
